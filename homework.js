...
async function addToCartWithAxios(productId, quantity) {
  ...
  return response.data; // 回傳整個 response.data，裡面包含購物車資料
}
...
1. 語法差異：fetch 需要手動呼叫 .json() 解析回應，axios 自動解析 JSON

2. 錯誤處理：fetch 只有在網路錯誤時才進 catch（4xx/5xx 不會進 catch），axios 只要非 2xx 狀態碼就會進 catch
3. 功能豐富度：fetch 是瀏覽器原生 API 不須安裝，axios 是第三方套件需要安裝但支援更多功能（請求取消、攔截器、超時設定等）
...
if (require.main === module) {
  ...
  async function runTests() {
    ...
    if (API_PATH) {
      ...
      // }
      // try {
      //   const result = await addToCartWithAxios("EoFimcTKdq10kCLUHBFy",3);
    ...
    //        console.log('第一個產品 ID:', products[0].id);
    //        console.log('第一個產品名稱:', products[0].title);
    //    }
    //   } catch (error) {
    //     console.log('getProductsWithAxios 錯誤:', error.message);
    // }
